...
def geocode_google(endereco, api_key):
    ...
    st.write(f"🔍 Endereço consultado: {endereco}")
    st.write(f"📦 Resposta da API: {data}")
    if data["status"] == "OK":
...
API_KEY = "AIzaSyDg3Hdn_Is-01dMGEHDRmOBnASbProZJ7w"
...
if st.button("Testar geocodificação"):
    lat, lon = geocode_google(endereco_teste, API_KEY)
